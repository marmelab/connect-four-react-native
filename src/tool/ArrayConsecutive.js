...
const twoDimensionalArrayHasConsecutiveValues = (array, numberOfConsecutiveValues = 4) => {
    let which;

    const areConsecutive = array.some((chunk) => {
        const { areConsecutive: chunkConsecutive, which: chunkWhich } = arrayHasConsecutiveValues(chunk, numberOfConsecutiveValues);
        if (chunkConsecutive) {
            which = chunkWhich;
            return true;
        }
